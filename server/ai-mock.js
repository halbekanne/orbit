...
const FINDINGS = {
  ...
  a11yMissingRole: {
    ...
    line: 12,
    detail:
      'Der Schließen-Button ist ein <div> ohne Rolle, ohne zugänglichen Namen und ohne Tastaturunterstützung. Screenreader-Nutzer können dieses Element weder finden noch bedienen.',
    suggestion:
      'Semantisches <button>-Element verwenden und aria-label ergänzen: <button aria-label="Dialog schließen" @click=${this.onClose}>✕</button>',
  ...
  a11yShadowRef: {
    ...
    line: 28,
    detail:
      'aria-labelledby referenziert eine ID außerhalb des Shadow DOM. Diese Referenz funktioniert nicht über Shadow-Grenzen hinweg — der zugängliche Name bleibt leer.',
...
const SCENARIOS = [
      ...
      const decisions = [
        {
          agent: 'ak-abgleich',
          action: 'kept',
          reason: 'Klares AK-Gap, Kritisch',
          finding: FINDINGS.akAbgleich.title,
        },
        {
          agent: 'code-quality',
          action: 'kept',
          reason: 'Typ-Sicherheit relevant',
          finding: FINDINGS.codeQuality1.title,
        },
        {
          agent: 'code-quality',
          action: 'removed',
          reason: 'Duplikat / zu geringfügig',
          finding: FINDINGS.codeQuality2.title,
        },
      ];
      const rawResponse = {
      ...
      const decisions = [
        {
          agent: 'code-quality',
          action: 'kept',
          reason: 'Memory Leak ist relevant',
          finding: FINDINGS.eventListener.title,
        },
        {
          agent: 'code-quality',
          action: 'kept',
          reason: 'Toter Code sollte entfernt werden',
          finding: FINDINGS.nullCheck.title,
        },
      ];
      const rawResponse = {
        findings: cqFindings,
  ...
  },
  {
    ...
    async run(emit) {
      ...
      emit('agent:error', {
        agent: 'ak-abgleich',
        error: 'AI API error: 503 — Service Unavailable',
      });
      ...
      const decisions = [
        {
          agent: 'code-quality',
          action: 'kept',
          reason: 'Valides Styling-Problem',
          finding: FINDINGS.shadowDom.title,
        },
      ];
      const rawResponse = {
        findings: cqFindings,
      ...
      emit('consolidator:done', {
        ...
        result: {
          ...
          summary: '1 Auffälligkeit: 1 Wichtig',
          warnings: [
            'Agent 1 (AK-Abgleich) fehlgeschlagen: AI API error: 503 — Service Unavailable',
          ],
          reviewedAt: new Date().toISOString(),
        },
        decisions,
  ...
  },
  {
    ...
    async run(emit) {
      ...
      const consolidatedFindings = [
        FINDINGS.codeQuality1,
        FINDINGS.a11yMissingRole,
        FINDINGS.a11yShadowRef,
      ];
      const decisions = [
        {
          agent: 'code-quality',
        ...
        },
        {
          agent: 'accessibility',
          action: 'kept',
          reason: 'Kritisches Barrierefreiheitsproblem',
          finding: FINDINGS.a11yMissingRole.title,
        },
        {
          agent: 'accessibility',
          action: 'kept',
          reason: 'ARIA-Referenz funktioniert nicht über Shadow-Grenzen',
          finding: FINDINGS.a11yShadowRef.title,
        },
      ];
      const rawResponse = {
        findings: consolidatedFindings,
