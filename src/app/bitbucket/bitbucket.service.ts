...
export class BitbucketService {
  ...
  readonly myPullRequests = computed(() => {
    ...
    const sortOrder = (pr: PullRequest): number => {
      ...
      if (pr.myReviewStatus === 'Changes Requested' || pr.myReviewStatus === 'Needs Re-review')
        return 1;
  ...
  getReviewerPullRequests(): Observable<PullRequest[]> {
    return this.http
      .get<BitbucketPrPageRaw>(`${this.baseUrl}/dashboard/pull-requests`, {
        params: new HttpParams().set('role', 'REVIEWER').set('state', 'OPEN').set('limit', '50'),
      })
      .pipe(
        map((page) =>
          page.values.map((pr) => this.mapPr(pr, this.settingsService.bitbucketConfig().userSlug)),
        ),
      );
  }
  ...
  getAuthoredPullRequests(): Observable<PullRequest[]> {
    return this.http
      .get<BitbucketPrPageRaw>(`${this.baseUrl}/dashboard/pull-requests`, {
        params: new HttpParams().set('role', 'AUTHOR').set('state', 'OPEN').set('limit', '50'),
      })
  ...
  getReviewerPrActivityStatus(
    pr: Pick<PullRequest, 'prNumber' | 'toRef'>,
  ): Observable<'Changes Requested' | 'Needs Re-review'> {
  ...
  getDiffstat(
    ...
    repoSlug: string,
    prId: number,
  ): Observable<{ additions: number; deletions: number; total: number }> {
    return this.http
      .get<{
        additions: number;
        deletions: number;
      ...
      .pipe(
        catchError(() =>
          of(undefined as unknown as { additions: number; deletions: number; total: number }),
        ),
      );
  }
  ...
  private mapPr(raw: BitbucketPrRaw, currentUserSlug: string): PullRequest {
    ...
    const finalStatus: PrStatus =
      myReviewStatus === 'Awaiting Review' && otherApproved ? 'Approved by Others' : myReviewStatus;
