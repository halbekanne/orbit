...
import { JiraTicket, PullRequest } from '../../shared/work-item.model';
...
describe('PrDetailComponent', () => {
  ...
  const mockAiReview = {
    reviewState: signal<import('../../review/review.model').ReviewState>('idle'),
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: AiReviewService, useValue: mockAiReview },
        {
          ...
          useValue: { selectedItem: signal(null), demoteToIdea: vi.fn(), promoteToTodo: vi.fn() },
        },
        {
          provide: FocusService,
          useValue: { isFocused: () => false, setFocus: vi.fn(), clearFocus: vi.fn() },
        },
        { provide: TodoService, useValue: { update: vi.fn(), todos: signal([]) } },
        { provide: IdeaService, useValue: { update: vi.fn() } },
        {
          provide: SettingsService,
          useValue: { aiReviewsEnabled: signal(true), theme: signal('system') },
        },
      ],
    });
  });
  ...
  it('shows error state when fetch fails', async () => {
    ...
    const jiraButton = Array.from<Element>(fixture.nativeElement.querySelectorAll('button')).find(
      (b) => b.textContent!.includes('Jira-Ticket'),
