  ...
  template: `
    ...
    >
        ...
        @switch (displayPhase()) {
          @case ('morning-open') {
            <div class="flex items-start justify-between gap-2 mb-1">
              <span
                class="text-[10px] font-semibold uppercase tracking-wider text-[var(--color-primary-solid)]"
                >Tagesfokus</span
              >
              ...
              <div class="min-w-0">
                @if (displayEntry()?.morningQuestion) {
                  <p class="font-serif italic text-xs text-[var(--color-text-muted)] mb-0.5">
                    {{ displayEntry()!.morningQuestion }}
                  </p>
                }
                @if (displayEntry()?.morningFocus) {
                  <p
                    class="font-serif italic text-[13px] text-[var(--color-text-body)] line-clamp-2"
                  >
                    {{ displayEntry()!.morningFocus }}
                  </p>
                }
              </div>
            </div>
          ...
          @case ('evening-open') {
            <div class="flex items-start justify-between gap-2 mb-1">
              <span class="text-[10px] font-semibold uppercase tracking-wider text-amber-600"
                >Tagesreflektion</span
              >
              <span class="text-xs text-[var(--color-text-muted)]">{{ weekday() }}</span>
            </div>
            <div class="flex items-center gap-2">
          ...
          @case ('evening-filled') {
            <div class="flex items-start justify-between gap-2 mb-1">
              <span class="text-[10px] font-semibold uppercase tracking-wider text-emerald-600"
                >Tag abgeschlossen</span
              >
              <span class="text-xs text-[var(--color-text-muted)]">{{ weekday() }}</span>
            </div>
            <div class="flex items-start gap-2">
              ...
              <div class="min-w-0">
                ...
                @if (displayEntry()?.eveningReflection) {
                  <p
                    class="font-serif italic text-[13px] text-[var(--color-text-body)] line-clamp-2"
                  >
                    {{ displayEntry()!.eveningReflection }}
                  </p>
                }
      ...
      <div
        ...
        [class.bg-violet-500]="
          displayPhase() === 'morning-open' || displayPhase() === 'morning-filled'
        "
        [class.bg-stone-800]="
          displayPhase() === 'evening-open' || displayPhase() === 'evening-filled'
        "
        aria-hidden="true"
      >
        <svg
...
export class ReflectionCardComponent {
  ...
  readonly pulseAnimation = computed(() => {
    ...
    if (phase === 'morning-open')
      return 'var(--pulse-animation, gentlePulse 3s ease-in-out infinite)';
    if (phase === 'evening-open')
      return 'var(--pulse-animation, gentlePulseAmber 3s ease-in-out infinite)';
    return 'none';
  });
