...
import { DayAppointment } from '../../calendar/day-schedule.model';
import { DayTimelineComponent } from '../../calendar/day-timeline/day-timeline';
import { AppointmentPopupComponent } from '../../calendar/appointment-popup/appointment-popup';
...
@Component({
  ...
  template: `
    @switch (viewState()) {
      @case ('input') {
        <div
          class="h-full flex items-start justify-center pt-12 px-6"
          [class.anim-page-out]="pageTransitioning()"
        >
          <div class="w-full max-w-[460px]">
            ...
            @if (!isMorning()) {
              @if (entry()?.morningFocus) {
                <div class="mb-6 border-l-[3px] border-[var(--color-primary-border)] pl-4">
                  <p class="font-serif italic text-sm text-[var(--color-primary-solid)] mb-1">
                    {{ entry()!.morningQuestion }}
                  </p>
                  <p
                    class="font-serif italic text-base text-[var(--color-text-body)] leading-relaxed"
                  >
                    {{ entry()!.morningFocus }}
                  </p>
                </div>
            ...
            <div
              ...
              [class]="
                isMorning()
                  ? 'border-l-[3px] border-[var(--color-primary-solid)] pl-4'
            ...
            <textarea
              ...
              [class]="
                isMorning()
                  ? 'focus:ring-2 focus:ring-[var(--color-focus-ring)] focus:border-[var(--color-primary-border)]'
            ...
            <div class="flex gap-3 mt-4">
              <button
                ...
                [class]="
                  isMorning()
                    ? 'bg-[var(--color-primary-solid)] hover:bg-[var(--color-primary-solid-hover)] focus-visible:outline-[var(--color-focus-ring)]'
                    : 'bg-stone-800 hover:bg-stone-900 focus-visible:outline-[var(--color-text-body)]'
                "
      ...
      @case ('calendar-setup') {
        <div class="h-full flex flex-col items-center pt-6 px-6 anim-page-in">
          ...
          <div
            class="flex-1 w-full max-w-[520px] min-h-0 border border-[var(--color-border-subtle)] rounded-xl overflow-hidden bg-[var(--color-bg-card)]"
          >
            <app-day-timeline
            ...
            @if (readonlyMorning()) {
              ...
              @if (entry()?.morningFocus) {
                <div class="bg-[var(--color-bg-surface)] rounded-2xl px-7 py-6 mb-8">
                  <p
                    class="font-serif italic text-xl text-[var(--color-text-body)] leading-relaxed"
                  >
                    {{ entry()!.morningFocus }}
                  </p>
                </div>
              ...
              @if (entry()?.morningFocus) {
                <div class="mb-2 border-l-[3px] border-[var(--color-primary-border)] pl-5">
                  <p
                    class="font-serif italic text-lg text-[var(--color-text-muted)] leading-relaxed"
                  >
                    {{ entry()!.morningQuestion }}
                  </p>
                </div>
...
export class ReflectionDetailComponent {
  ...
  readonly animPhase = signal<'form-exit' | 'circle' | 'check' | 'text' | 'hold' | 'fade-out'>(
    'form-exit',
  );
  ...
  readonly calendarPopupState = signal<{
