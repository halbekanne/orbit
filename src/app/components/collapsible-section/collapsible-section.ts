  ...
  template: `
    <div class="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border-subtle)] shadow-sm overflow-hidden">
      ...
      >
        ...
        <span class="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">{{ label() }}</span>
        <ng-content select="[sectionMeta]" />
      </button>
      @if (expanded()) {
        <div class="border-t border-[var(--color-border-subtle)]" [class]="noPadding() ? '' : 'px-6 py-4'">
          <ng-content />
        </div>
      }
    </div>
