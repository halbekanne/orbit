    ...
    >
      ...
      @for (apt of appointments(); track apt.id) {
        <div
          ...
          [style.height.px]="
            minutesToPx(timeToMinutes(apt.endTime)) - minutesToPx(timeToMinutes(apt.startTime))
          "
        ...
        >
          ...
          <div style="padding: 3px 6px; pointer-events: none;">
            <div
              style="font-size: 12px; font-weight: 600; color: var(--color-primary-text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"
            >
              {{ apt.title }}
            </div>
            <div style="font-size: 11px; color: var(--color-primary-solid);">
      ...
      @if (dragPreview() !== null) {
        <div
          ...
          [style.height.px]="
            minutesToPx(dragPreview()!.endMinutes) - minutesToPx(dragPreview()!.startMinutes)
          "
        >
...
export class DayTimelineComponent implements OnInit, OnDestroy {
  ...
  private pointerToMinutes(event: PointerEvent): number {
    ...
    const rawMinutes =
      START_HOUR * 60 + (clampedY / ((END_HOUR - START_HOUR) * HOUR_HEIGHT)) * TOTAL_MINUTES;
