...
import { PipelineState } from '../review.model';
...
describe('ReviewPipelineComponent', () => {
  ...
  it('shows running indicator for active agents', () => {
    const fixture = setup({
      agents: [{ agent: 'ak-check', label: 'AK-Abgleich', temperature: 0.2, status: 'running' }],
      consolidator: { status: 'pending' },
      warnings: [],
    });
    openSection(fixture);
  ...
  it('shows temperature badges', () => {
    const fixture = setup({
      agents: [{ agent: 'ak-check', label: 'AK-Abgleich', temperature: 0.4, status: 'done' }],
      consolidator: { status: 'pending' },
      warnings: [],
    });
    openSection(fixture);
  ...
  it('shows consolidator decisions when present', () => {
    const fixture = setup({
      agents: [{ agent: 'ak-check', label: 'AK-Abgleich', temperature: 0.2, status: 'done' }],
      consolidator: {
        status: 'done',
        temperature: 0.1,
        duration: 1.5,
  ...
  it('shows agent description for known agents', () => {
    const fixture = setup({
      agents: [{ agent: 'code-quality', label: 'Code-Qualität', temperature: 0.4, status: 'done' }],
      consolidator: { status: 'pending' },
      warnings: [],
    });
    openSection(fixture);
    ...
    expect(el.textContent).toContain(
      'Prüft allgemeine Code-Qualität, Patterns und potenzielle Fehler',
    );
  });
  ...
  it('shows merged details toggle for agent with thoughts and raw response', () => {
    ...
    openSection(fixture);
    const el = fixture.nativeElement as HTMLElement;
    const detailsBtn = Array.from(el.querySelectorAll('button')).find((b) =>
      b.textContent?.includes('Details anzeigen'),
    );
