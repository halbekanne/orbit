    ...
    <app-collapsible-section label="KI-Review" [expanded]="true" [noPadding]="true">
      ...
      <ng-container sectionMeta>
        @if (review === 'idle') {
          ...
          <span class="ml-auto font-mono text-xs text-[var(--color-text-muted)]">{{
          ...
          @if (severityCounts(); as counts) {
            <span class="flex items-center gap-1.5 ml-1">
              @if (counts.critical > 0) {
                <orbit-badge class="pop-in" color="danger"
                  >{{ counts.critical }} Kritisch</orbit-badge
                >
              }
              @if (counts.important > 0) {
                <orbit-badge class="pop-in" color="signal" style="animation-delay: 0.08s"
                  >{{ counts.important }} Wichtig</orbit-badge
                >
              }
              @if (counts.minor > 0) {
                <orbit-badge class="pop-in" color="neutral" style="animation-delay: 0.16s"
                  >{{ counts.minor }} Gering</orbit-badge
                >
              }
            </span>
          }
          @if (review.pipeline.totalDuration != null) {
            <span class="ml-auto font-mono text-xs text-[var(--color-text-muted)]">{{
              formatDuration(review.pipeline.totalDuration!)
            }}</span>
          }
        }
      </ng-container>
      @if (review !== 'idle' && review.status === 'running') {
                ...
                @if (agent.status === 'done') {
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex items-baseline gap-2 flex-wrap slide-in">
                      ...
                      <span
                        class="text-[11px] px-1.5 py-0.5 rounded bg-[var(--color-bg-surface)] text-[var(--color-text-muted)] border border-[var(--color-border-subtle)]"
                  ...
                  <span
                    class="mt-[3px] w-4 h-4 shrink-0 rounded-full bg-violet-400 pulse-dot"
                    aria-hidden="true"
                  ></span>
                  <div class="min-w-0 flex-1">
                    <div class="flex items-baseline gap-2 flex-wrap">
                      ...
                      <span
                        class="flex items-center gap-0.5 text-xs text-[var(--color-primary-solid)]"
                      >
                  ...
                  <span
                    class="mt-[3px] w-4 h-4 shrink-0 rounded-full border-2 border-dashed border-[var(--color-border-default)]"
                    aria-hidden="true"
                  ></span>
            ...
            @if (review.pipeline.consolidator.status !== 'pending') {
              <div class="flex gap-3">
                @if (review.pipeline.consolidator.status === 'done') {
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex items-baseline gap-2 flex-wrap slide-in">
                      ...
                      @if (review.pipeline.consolidator.duration != null) {
                        <span class="font-mono text-xs text-[var(--color-text-muted)]">{{
                          formatDuration(review.pipeline.consolidator.duration!)
                        }}</span>
                      }
                    </div>
                  </div>
                  ...
                  <span
                    class="mt-[3px] w-4 h-4 shrink-0 rounded-full bg-red-500"
                    aria-hidden="true"
                  ></span>
                  <div class="min-w-0 flex-1">
                    <div class="flex items-baseline gap-2 flex-wrap">
        ...
        <div class="flex flex-col items-center justify-center py-12 px-6">
          ...
          <p class="text-sm text-[var(--color-text-muted)] text-center max-w-xs mb-5">
            Der Review-Service war nicht erreichbar. Das kann an einer kurzzeitigen Störung liegen —
          ...
          <details class="mt-6 w-full max-w-md">
            <summary
              class="text-xs text-[var(--color-text-muted)] cursor-pointer hover:text-[var(--color-text-body)]"
            >
              Technische Details
            </summary>
            <pre
              class="mt-2 bg-[var(--color-bg-surface)] border border-[var(--color-border-subtle)] text-[var(--color-text-body)] font-mono text-xs p-3 rounded-md overflow-x-auto whitespace-pre-wrap"
              >{{ review.message }}</pre
            >
          </details>
        </div>
        ...
        <div class="px-5 py-4">
          <div
            class="flex items-start gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200 mb-4"
          >
            <svg
          ...
          @if (review.data.findings.length === 0) {
            ...
            <div class="space-y-3 mt-3">
              @for (group of fileGroups(); track group.file) {
                <div
                  [attr.data-file-group]="group.file"
                  class="bg-[var(--color-bg-surface)] rounded-lg border border-[var(--color-border-subtle)]"
                >
                  <button
                    type="button"
                      ...
                      @for (finding of group.findings; track $index) {
                        <div class="pl-3 py-2" [class]="findingStripeClass(finding.severity)">
                          <div class="flex items-center gap-2 mb-1 flex-wrap">
                            <orbit-badge [color]="severityColor(finding.severity)" size="sm">{{
                            ...
                            <span
                              class="font-mono text-xs text-[var(--color-text-muted)] ml-auto pr-2"
                              >Zeile {{ finding.line }}</span
                            >
                          </div>
                          <p class="text-sm font-medium text-[var(--color-text-heading)] mb-1">
                            {{ finding.title }}
                          ...
                          @if (finding.codeSnippet) {
                            <pre
                              class="font-mono text-xs bg-[var(--color-bg-surface)] border border-[var(--color-border-subtle)] text-[var(--color-text-body)] rounded px-3 py-2 mb-1.5 overflow-x-auto whitespace-pre-wrap"
                              >{{ finding.codeSnippet }}</pre
                            >
                          }
                          <div
                            class="bg-[var(--color-bg-surface)] rounded px-3 py-2 text-sm text-[var(--color-text-body)] leading-relaxed"
                            [innerHTML]="finding.detail | inlineCode"
                          ></div>
                          @if (finding.suggestion) {
                            <div class="mt-1.5 text-sm text-[var(--color-text-body)]">
                              <span class="font-medium text-[var(--color-text-body)]"
                                >Vorschlag:</span
                              >
        ...
        <div class="flex flex-col items-center justify-center py-12 px-6">
          <div class="relative w-24 h-24 mb-6">
            <div
              class="absolute inset-0 rounded-full border-2 border-dashed border-[var(--color-border-subtle)] orbit-breathe"
            ></div>
            <div
...
export class ReviewFindingsComponent {
  ...
  readonly progressInfo = computed(() => {
    ...
    const done =
      agents.filter((a) => a.status === 'done' || a.status === 'error').length +
  ...
  severityDotClass(severity: string): string {
    switch (severity) {
      case 'critical':
        return 'bg-[var(--color-danger-solid)]';
      case 'important':
        return 'bg-[var(--color-signal-bar)]';
      default:
        return 'bg-[var(--color-badge-neutral-dot)]';
    }
  }
  ...
  findingStripeClass(severity: string): string {
    switch (severity) {
      case 'critical':
        return 'border-l-[3px] border-red-500 rounded-r-md bg-red-50/30';
      case 'important':
        return 'border-l-[3px] border-amber-500 rounded-r-md bg-amber-50/20';
      default:
        return 'border-l-[3px] border-stone-400 rounded-r-md';
    }
  }
