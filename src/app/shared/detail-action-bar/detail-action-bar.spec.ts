...
import { ReviewState } from '../../review/review.model';
...
describe('DetailActionBarComponent', () => {
  ...
  let mockAiReview: {
    reviewState: ReturnType<typeof signal<ReviewState>>;
    canReview: ReturnType<typeof signal<boolean>>;
  ...
  describe('PR', () => {
    ...
    it('shows "Review läuft..." and disables button during review', () => {
      ...
      fixture.detectChanges();
      const btn = Array.from(el.querySelectorAll('button')).find((b) =>
        b.textContent!.includes('Review läuft'),
      );
    ...
    it('disables KI-Review when canReview is false', () => {
      ...
      fixture.detectChanges();
      const btn = Array.from(el.querySelectorAll('button')).find((b) =>
        b.textContent!.includes('KI-Review starten'),
  ...
  describe('Todo (open)', () => {
    ...
    it('calls todoService.update with done status on Erledigt click', () => {
      ...
      expect(updateTodoSpy).toHaveBeenCalledWith(
        expect.objectContaining({ id: 'td1', status: 'done' }),
      );
    });
    ...
    it('toggles urgent on Dringend click', () => {
      ...
      expect(updateTodoSpy).toHaveBeenCalledWith(
        expect.objectContaining({ id: 'td1', urgent: true }),
      );
    });
    ...
    it('sets wont-do status on Nicht erledigen click', () => {
      ...
      expect(updateTodoSpy).toHaveBeenCalledWith(
        expect.objectContaining({ id: 'td1', status: 'wont-do' }),
      );
    });
  ...
  describe('Todo (done)', () => {
    ...
    it('reopens todo on Wieder öffnen click', () => {
      ...
      expect(updateTodoSpy).toHaveBeenCalledWith(
        expect.objectContaining({ id: 'td1', status: 'open', completedAt: null }),
      );
    });
  });
  ...
  describe('Todo (wont-do)', () => {
    ...
    it('reopens todo on click', () => {
      ...
      clickButton('Wieder öffnen');
      expect(updateTodoSpy).toHaveBeenCalledWith(
        expect.objectContaining({ id: 'td1', status: 'open' }),
      );
    });
  });
  ...
  describe('Idea (active)', () => {
    ...
    it('sets wont-do on Nicht verfolgen click', () => {
      ...
      expect(updateIdeaSpy).toHaveBeenCalledWith(
        expect.objectContaining({ id: 'id1', status: 'wont-do' }),
      );
    });
  });
  ...
  describe('Idea (wont-do)', () => {
    ...
    it('revives idea on click', () => {
      ...
      expect(updateIdeaSpy).toHaveBeenCalledWith(
        expect.objectContaining({ id: 'id1', status: 'active' }),
      );
    });
  });
    ...
    it('shows "Fokus entfernen" when item is focused', () => {
      ...
      TestBed.configureTestingModule({
        ...
        providers: [
          {
            provide: WorkspaceService,
            useValue: { selectedItem: signal(null), promoteToTodo: vi.fn(), demoteToIdea: vi.fn() },
          },
