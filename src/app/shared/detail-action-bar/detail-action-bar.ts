...
const BTN =
  'inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-all duration-100 cursor-pointer select-none whitespace-nowrap';
        ...
        @case ('pr') {
          ...
          @if (settingsService.aiReviewsEnabled()) {
            ...
            <button
              ...
              [disabled]="
                (review !== 'idle' && review.status === 'running') || !aiReview.canReview()
              "
