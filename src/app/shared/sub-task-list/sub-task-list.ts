        ...
        @for (subtask of subtasks(); track subtask.id; let i = $index) {
          ...
          >
            ...
            <button
              ...
              [attr.aria-label]="
                subtask.status === 'done' ? 'Als offen markieren' : 'Als erledigt markieren'
              "
              ...
              <span
                ...
                [class]="
                  subtask.status === 'done'
                    ...
                    : 'text-[var(--color-text-heading)] hover:text-[var(--color-primary-text)]'
                "
                (click)="startEdit(i)"
                tabindex="0"
...
export class SubTaskListComponent {
  ...
  toggleSubtask(index: number, event: Event): void {
    ...
    if (toggled.status === 'done') {
      ...
      const anchor =
        (btn
          .closest('[data-testid="subtask-item"]')
          ?.querySelector('.st-checkbox') as HTMLElement) ?? btn;
      try {
