...
describe('SettingsService', () => {
  ...
  it('should load status and settings when configured', async () => {
    ...
    httpTesting.expectOne(r => r.url.endsWith('/api/settings/status')).flush({ configured: true });
  ...
  it('should not load settings when not configured', async () => {
    const loadPromise = service.load();
    httpTesting.expectOne(r => r.url.endsWith('/api/settings/status')).flush({ configured: false });
    await loadPromise;
  ...
  it('should save settings and update state', async () => {
    ...
    httpTesting.expectOne(r => r.method === 'PUT' && r.url.endsWith('/api/settings')).flush(settings);
    await savePromise;
