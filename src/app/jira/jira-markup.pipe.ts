...
function extractBlocks(text: string, stash: string[]): string {
  const protect = (html: string) => {
    stash.push(html);
    return `\x00BLOCK:${stash.length - 1}\x00`;
  };
  ...
  text = text.replace(/\{code(?::([^}]*))?\}([\s\S]*?)\{code\}/gi, (_, params, inner) => {
    ...
    return protect(
      `<pre class="jira-code-block">${titleHtml}<code class="hljs">${highlighted}</code></pre>`,
    );
  });
...
function processLines(text: string): string {
  ...
  for (const rawLine of lines) {
    ...
    if (bqm) {
      ...
      flushTable();
      out.push(
        `<blockquote class="jira-quote"><p>${applyInline(escapeHtml(bqm[1]))}</p></blockquote>`,
      );
      continue;
    }
